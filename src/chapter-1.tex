...
in no particular order. Every explicitly omitted proof in this chapter can be found in any of these
references.
...
\begin{definition}
	Let $M$ be a topological manifold. We call two charts $(\U, x), (\V, y)$ \textbf{smoothly
	compatible} if $x(\U \cap \V)$ and $y(\U \cap \V)$ are open subsets of $\RR^n$ and $x \circ
	y^{-1}$ and $y \circ x^{-1}$ are smooth as functions on $\RR^n$.
\end{definition}
Using the chain rule, it is easy to prove that a map is smooth with respect to two charts if
and only if it is smooth with respect to all smoothly compatible charts. Adding a choice of
smoothly compatible charts on a manifold gives us a new object:
\begin{definition}
A \textbf{smooth manifold} is a topological manifold $M$ with a choice of pairwise smoothly
compatible charts $\{(\U_\alpha, x_\alpha)\}_{\alpha \in I}$ such that $\bigcup_{\alpha \in I}
\U_\alpha = M$.
\end{definition}
Given smooth manifolds, our suggested definition of smoothness makes sense:
\begin{definition}
Given two smooth manifolds $M, N$ we call a function $M \to N$ \textbf{smooth} if for all
choices of coordinates $(\U, x)$ of $M$ and $(\V, y)$ of $N$ the function
\[
	y \circ f \circ x^{-1}: x(f^{-1}(\V)) \to y(\U)
\]
is smooth.
\end{definition}
Unsurprisingly smooth functions are the natural maps between smooth manifolds and hereinafter
every map between smooth manifolds is assumed to be smooth unless stated otherwise. Smoothness
is also used to define an equivalence relation on manifolds that is more useful than strict
equality:
\begin{definition}
	Let $M,N$ be smooth manifolds. We call a smooth map $f: M \to N$ \textbf{diffeomorphism} if it is
	bijective and its inverse is also smooth. Two manifolds that admit a diffeomorphism between them are called \textbf{diffeomorphic}.
\end{definition}
...
\[
	D_{x(p)} (x \circ y^{-1}): T^x_p M \to T^y_p M
\]
(This is an isomorphism since $x \circ y^{-1}$ is a diffeomorphism). Then one can avoid any choice of
coordinates by defining
\[
	T_p M \coloneqq
		{}^{\displaystyle \bigoplus_{(\U, x) \text{ chart around } p} T^x_p M}
			\Bigg/
			{}_{\displaystyle \sim }
\]
with $v \sim w$ if and only if $w = (x \circ y^{-1})(v)$ for $v \in T^x_p M, w \in T^y_p M$ for
some charts $(\U, x), (\V, y)$ around $p$.

This is isomorphic to every $T^x_p M$, but has no canonical basis (but choosing local
coordinates $(\U, x)$ induces one, which with a little abuse of notation we also denote
$\frac{\partial}{\partial x_1}, \dots, \frac{\partial}{\partial x_n}$).
Using these infos we are able to define the derivative itself. For any given charts $(x, \U)$
around $p$ and $(y, \V)$ around $f(p)$ there is a derivative of $f$ with respect to these
...
\end{definition}
To ensure that this map is well-defined we have to prove that any map fulfilling this equality for charts
...
\end{align*}
In this equation $D_{x(p)}(\tilde{x} \circ x^{-1})$ is the coordinate changing isomorphism
between $T^x_p M$ and $T^{\tilde{x}}_p M$, hence $D_{x(p)}(\tilde{x} \circ x^{-1}) \circ
(i^x_p)^{-1}$ is just $(i^{\tilde{x}}_p)^{-1}$. This can also be done for $D_{\tilde{y}(f(p))}
(y \circ \tilde{y}^{-1})$,  resulting in
\begin{align*}
...
\end{align*}
Thus $T_p f$ is well-defined.

The theory surrounding tangent spaces is rich and interesting, but we only have time to scratch its surface,
...
\label{sec:diff_forms}
Having developed the theory of tangent spaces, we can start translating our introductory
motivation from $\RR^n$ to general manifolds. Our first example was about the relationship
between the connected components of open subsets of $\RR^n$ and the smooth functions with
vanishing derivative defined on it. The fact that locally constant functions are related to
connected component directly translates to smooth manifolds, all that is left is to find a
nice description of these using derivatives. This is a bit more involved: The tangent map of
a smooth function $f: M \to \RR$ is not going to vanish if $f$ is locally constant,  because
$Tf$ not only contains information about the change of $f(p)$, but also about $f(p)$ itself. In
the case of general manifolds this is necessary, but in the case of $\RR$ there is a canonical
isomorphism between $T \RR$ and $\RR \times \RR$ (with the first component representing points
and the second tangent vectors). This isomorphism can be used to project $T f$ to the second
component (i.e. to the "change"). We call this projection the \textbf{differential of $f$} and
denote it by $\rmd f$. Then $f$ is locally constant if and only if its differential is zero
everywhere, letting us describe the locally constant functions as the kernel of $\rmd$, which thus
has as many dimensions as $M$ has connected components.
...
Differential forms are needed for integration on manifolds since by itself a manifold has no
canonical notion of area (e.g which value should $\int_{S^1} 1\, \rmd x$ take?). While one
could define integrals by adding data to the manifold itself (e.g. a measure), differential
forms offer an alternative approach, including these informations in the integrands themselves.
They do that by mapping each point $p$ to a local notion of volume, though in a way that needs
a bit of explanation. They give this local notion of volume around $p$ by assigning volume to
parallelepipeds based at $p$. Of course parallelepipeds do not exist on arbitrary manifolds,
but one can approximate them through the use of tangent vectors i.e. every $n$-tuple of tangent
vectors in $T_p M$ can be interpreted as an $n$-parallelepiped. Thus we want to describe
volume using maps from $(T_p M)^n$ to $\RR$, though we also want them to fulfill some more
requirements to make them more "volume-like". Firstly we are going to require them to map
linear dependent tangent vectors to $0$ since these represent degenerate parallelepipeds.
Secondly we want them to be multi-linear, like the normal volume of parallelepipeds.

This has the odd consequence of having to deal with negative "volume", but this can be seen as a feature
instead of a flaw by interpreting signed volume as a volume (the absolute value) together
with an \textit{orientation} (the sign). This Orientation can be intuitively understood as
some additional data added to a space giving a sense of direction. This sense of direction is
probably easiest to understand in one dimension, where it is really just a choice of direction
e.g. to go clockwise or counterclockwise around a circle. In higher dimensions it is a bit
more involved and can be best understood as choosing an in-/outside e.g. choosing on a plane
which side is up. One can properly define what it means for a smooth manifold to be oriented and
if we would want to actually integrate differentials forms we would have to, but similiar as
with integration itself, we are just going to use it as a conceptual motivation.

Turning all this motivation into proper definitions requires some multi-linear algebra,
starting with reformulating the unwieldy requirement of mapping linear dependent vectors to
zero into one that is easier to deal with:
\begin{lemma}
A multi-linear map $f: V \times \dots \times V \to \RR$ is antisymmetric if and only if it maps every collection
of linearly dependent vectors to zero.
\end{lemma}
\begin{proof}
	We start with proving that any antisymmetric multi-linear function $f$ maps linearly dependent vectors to
...
where it comes from.
\end{remark}
We could now already define a differential $k$-form as a map from each point $p$ of a manifold $M$ to
an antisymmetric map $f: (T_p M)^k \to \RR$, but this would include too
many maps because we want differential forms to be smooth, though a priori it is not clear
what exactly this entails. But we are going to find a sensible definition of smoothness for
differential forms by developing a bit more (multi-)linear algebra. Recall that any given basis
$b_1, \dots, b_n$ of a vector space $V$ gives rise to a dual basis $e^1, \dots, e^n$  defined
...
\[
	...
	\end{cases}
\]
Denoting the dual space of $T_p M$ as $T^*_p M$, we can dualize every chart-induced basis
$\frac{\partial }{\partial x^1}, \dots \frac{\partial}{\partial x^n}$ to a basis of $T^*_p M$,
which we are going to denote as $\rmd x^1, \dots, \rmd x^n$. Then, given a "differential 1-form"
$\omega$ (i.e. a map that maps $p$ to $T^*_p M$), there are unique component functions $\omega_1,
\dots, \omega_n: M \to \RR$ for any chart $(\U, x)$ such that
\[
	\omega(p) = \omega_1(p) \rmd x^1 + \dots + \omega_n(p) \rmd x^n
\]
It then seems reasonable to call $\omega$ smooth if and only if its component functions are
smooth for any chart. Taking this definition to higher dimensions requires a representation
of higher dimensional multi-linear maps in a similar form, which in the special case of
antisymmetric maps can be achieved in a succinct way using the so called \textbf{wedge
product}. To define it one needs to start with defining the tensor product:
\begin{definition}  
	...
	\]		
\end{definition}
The tensor product of two antisymmetric maps is sadly not antisymmetric itself, but there is a way to
antisymmetrize any multi-linear map:
\begin{definition}
	Let $\alpha: V^k \to \RR$ be a multi-linear map. We define its antisymmetrization as
	\[
		\Alt(\alpha)(v_1, \dots, v_k)
			\coloneqq \frac{1}{k!} \sum_{\sigma \in S_k} (-1)^{|\sigma|} \alpha(v_{\sigma(1)}, \dots, v_{\sigma(k)})
	\]
\end{definition}
It is easy to see that $\Alt(\alpha)$ is always antisymmetric and we can now use it (plus a
combinatorial factor) to define:
\begin{definition}
	Let $\alpha: V^k \to \RR, \beta: V^\ell \to \RR$ be antisymmetric multilinear maps. We define their
	wedge product as
	\[
		\alpha \wedge \beta = \frac{(k + \ell)!}{k! \ell!} \Alt(\alpha \otimes \beta)
	\]
\end{definition}
Before doing anything with the wedge product, we are going to state (though not prove) some
of its elemental properties:
\begin{theorem}
	...
	\end{itemize}
\end{theorem}
We are going to use the wedge product mostly as a notational convenience, because it can be
used to together with a dual basis $\rmd x^1, \dots, \rmd x^n$ to form a basis of the $k$-fold
antisymmetric maps in the form of $\rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k}$ for all
$i_1 < \dots < i_k$. We can then extend the wedge product to differential forms as point-wise
multiplications, allowing us to write any differential form on a chart $(\U, x)$ as
\[
	\omega(p) = \sum_{i_1 < \dots < i_k} \omega_{i_1, \dots, i_k}(p) \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k}
\]
enabling us to again reduce smoothness to the smoothness of its components functions. Combining
all of our work results in:
\begin{definition}
	A \textbf{differential $k$-form} on an $n$-manifold $M$ is a map that assigns to any $p \in M$
	an antisymmetric $k$-fold multilinear function from $(T_pM)^k$ to $\RR$
	such that all its components functions with respect to all charts are smooth.

	The space of all differential $k$-forms on $M$ is denoted as $\Omega^k(M)$ and the space of all
	differential forms as 
	\[
		\Omega^*(M) \coloneqq \bigoplus_{k = 0}^\infty \Omega^k(M)
	\]
	We are also going to call this the \textbf{de Rham co-complex} of $M$.
\end{definition}
Note that this definition also includes differential 0-forms, which smoothly assign to every
point a function from the Cartesian product of 0 vector spaces (which is by definition just a
point) to $\RR$ i.e. they assign to every point a value from $\RR$. Thus differential 0-forms
are just the smooth functions on a manifold.

We actually already encountered examples of 1-forms:
\begin{example}
Given a smooth map $f: M \to N$, its differential $\rmd f: TM \to \RR$ restricted to any $T_p
M$ is a linear map and therefore can be seen as assigning a linear map to each point of $M$
i.e. it is a smooth 1-form. Even better, we can write this 1-form down explicitly for some local
chart $(\U, x)$:
\[
	\rmd f = \sum_{i = 1}^n \frac{\partial f}{\partial x^i} \rmd x^i
\]
...
\begin{definition}
Let $M$ be a smooth manifold. Define the \textbf{exterior derivative} $\rmd: \Omega^*(M) \to \Omega^*(M)$
as the unique linear operator mapping $k$-forms to $k + 1$-forms that fulfills:
\begin{itemize}
	...
	\item It is local, in the sense of $(\rmd \omega)(p)$ only depending on the values of $\omega$ on
		small neighborhoods of $p$.
\end{itemize}
\end{definition}
It is left to prove that such a map exists and is unique:
\begin{proof}
	Let us assume that a map $\rmd$ with all these properties does exist. We can then calculate
	$\rmd$ of a homogenous differential form $f \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k}$
	by using the graded leibniz rule, nil-potency and the fact that $\rmd$ agrees with the differential
	on zero-forms:
	\begin{align*}
		\rmd \left(f \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k}\right)
			&= \rmd f \wedge \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k} \\
			&= \sum_{i_1 < \dots < i_k} \rmd \omega_{i_1 \dots i_k} \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k} \\
			&= \sum_{j = 1}^n \frac{\partial}{\partial x^j} f \rmd x^j \wedge \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k} \\
	\end{align*}
	By linearity this completely determines $\rmd$ and thus proving it uniqueness and producing a concrete
	formular for $\rmd$. It only remains to show that this map does fullfill those properties and is
	independent of coordinates.

	This formula is by definition local and it is easy to see that it is indeed a linear operator
	and  matches the differential on smooth functions. It remains to prove the Leibniz rule
	and nilpotency. Because of linearity it is enough to prove the Leibniz rule for homogeneous
	terms $f \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k}$ and $g \rmd x^{j_1} \wedge \dots
	\wedge x^{j_\ell}$, which we are going to abbreviate as $f \rmd x^I$ and $g \rmd x^J$ for
	multi-indices $I,J$. A computation then leads to:
	\begin{align*}
...
\end{definition}
\begin{remark}
Defining de Rham cohomology as just a graded vector space hides the additional structure of
the wedge product. One could also define it as a vector space with product structure (a so
called algebra), but we are not going to do that since we are not going to define any product
structure on Čech cohomology (though this is possible and the cohomologies are isomorphic as
algebras, it just makes the proof more complicated without adding much more insight, for a
proof see chapter III of \cite{bott_differential_1982})
\end{remark}

As already discussed at the start of \ref{sec:diff_forms}, $H^0_\dr(M)$ consists of the locally
constant functions on $M$, hence we can already calculate it for any space:
\begin{theorem}
...
\begin{remark}
One might wonder how our second example of $\curl$-free vector fields fits into this framework
since 1-forms aren't vector fields. But 1-forms are dual to vector fields and thus there is
a canonical isomorphism between 1-forms and vector fields on $\RR^n$ induced by the euclidean
scalar product (or in general given by a riemannian metric). The exterior derivative of such a
1-form is then equivalent to the $\curl$ of the vector field and $curl$ free vector fields are
conservative if the first de Rham cohomology vanishes
\end{remark}
...
\end{lemma}
\begin{proof}
For $k > n$ the only antisymmetric $k$-form on a $n$-dimensional vector space is $0$, hence
$\Omega^k(M) = 0$, which implies $H^k_\dr(M) = 0$.
\end{proof}

Actual calculations of de Rham cohomology turn out to be a bit more involved:
\begin{lemma}
...
\end{lemma}
\begin{proof}
We are only going to prove this statement for the case of $n = 1$, referring to \cite{bott_differential_1982}
...
\]
Hence every 1-form on $\RR$ is the exterior derivative of a 0-form i.e. exact, ensuring that $H^1(\RR) = 0$.
\end{proof}
...
exact sequence, called the \textbf{Mayer-Vietoris sequence} between the cohomology of $M$ and
the cohomologies of $\U, \V, \U \cap \V$.
...
$f^*: \Omega^*(N) \to \Omega^*(M)$ as the linear function defined by
\[
	(f^* \omega)(p)(v_1, \dots, v_k) = \omega(f(p))(Tf(v_1), \dots, Tf(v_k))
\]
We call $f^* \omega$ the \textbf{pullback of $\omega$ via $f$} and denote its restriction to $\Omega^k(M)$
as $f^k$.
\end{definition}
...
\end{align*}
\end{proof}
As an interesting consequence of this statement we get that the pullback of any diffeomorphism
$f: M \to N$ is an isomorphism since $f^* \circ \left(f^{-1}\right)^* = (f \circ f^{-1})^* =
\mathrm{Id}^*$, which is easily seen to be the identity map on differential forms.
...
\end{theorem}
\begin{proof}
By (bi-)linearity and since both pullbacks and wedge products are defined pointwise it is enough to prove this for
terms $f \rmd x^I, g \rmd x^J$ with $I = (i_1, \dots, i_k), J = (j_1, \dots, j_\ell)$ being multi-indices.
Let $\varphi: M \to N$ be a smooth map, then
\begin{align*}
	\varphi^* (f \rmd x^I \wedge g \rmd x^J)(p)(v_1, \dots, v_{k + \ell})
		&= (f \rmd x^I \wedge g \rmd x^J)(\varphi(p))(T\varphi(v_1), \dots, T\varphi(v_{k + \ell})) \\
		&= (\varphi^*(f \rmd x^I) \wedge \varphi^* (g \rmd x^J))(p)(v_1, \dots, v_{k + \ell})
		 \qedhere
\end{align*}
\end{proof}
...
by restricting the charts of $M$ to $\U$.
\end{remark}

Having this out of the way, we can then derive the Mayer-Vietoris sequence from the inclusion maps:
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	& {\mathcal{U}} \\
	M && {\mathcal{U} \cap \mathcal{V}} \\
	& {\mathcal{V}}
	\arrow["{j_{\mathcal{U}}}", from=1-2, to=2-1]
	\arrow["{i_{\mathcal{U}}}", from=2-3, to=1-2]
	\arrow["{i_{\mathcal{V}}}"', from=2-3, to=3-2]
	\arrow["{j_{\mathcal{V}}}"', from=3-2, to=2-1]
\end{tikzcd}\]
All of these inclusions are smooth and hence induce a sequence of restrictions of differential forms:
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\Omega^*(M)} & {\Omega^*(\mathcal{U}) \bigoplus \Omega^*(\mathcal{V})} & {\mathcal{U} \cap \mathcal{V}}
	\arrow["{j^*_{\mathcal{U}} \oplus j^*_{\mathcal{V}}}", from=1-1, to=1-2]
	\arrow["{i^*_{\mathcal{U}} + i^*_{\mathcal{V}}}", from=1-2, to=1-3]
\end{tikzcd}\]
To be more succinct we are going to denote $j^*_{\mathcal{U}} \oplus j^*_{\mathcal{V}}$ as
$j^*$. This sequence being exact can be seen as the statement that the de Rham co-complex of a
space is the co-complex of its parts, modulo their overlap. Geometrically this seems reasonable,
one just has to modify some signs to make it work:
...
%
Proofing this statement requires the introduction of a very important tool of differential geometry:
\begin{definition}
Let $M$ be a smooth manifold with an open cover $\{\U_\alpha\}_{\alpha \in I}$. We call a
family of functions $\{\varphi_\alpha\}_{\alpha \in I}$ with every $\varphi_\alpha$ being a
smooth function from $M$ to $\RR$ a \textbf{partition of unity subordinate to
$\{\U_\alpha\}_{\alpha \in I}$} if
\begin{itemize}
...
\end{itemize}
\end{definition}
The usefulness of this definition critically depends on the existence of a partition of unity.
Luckily on  smooth manifolds this is always the case (though we won't prove it):
\begin{lemma}
...
\end{lemma}
Having partitions of unity at our disposal we can prove the statement:
\begin{proof}
...
partition of unity subordinate to $\U, \V$. Now $\rho_\V \omega$ (taken as pointwise
multiplication) can be zero-extended to $\U$, as can $\rho_\U \omega$ to $\V$. We then
have:
\begin{equation*}
...
\begin{align*}
	\rmd^k: A^k \to A^{k + 1}
\end{align*}
that are nilpotent i.e. $\rmd^{k + 1} \circ \rmd^k = 0$.

These maps together define a map $\rmd: A^* \to A^*$ in the obvious way and will be called
\textbf{differential} or \textbf{coboundary operator} of $A^*$.
\end{definition}
...
$k$-coboundaries of $A^*$.
\end{definition}
With the obvious examples being the space of differential forms on a manifold with the exterior
derivate as its differential and the de Rham cohomology.

No algebraic definition would be complete without defining maps between objects. In the case of cochain complexes
we want our maps to descend to cohomology, which motivates:
\begin{definition}[Cochain Map]
Let $(A^*, \rmd_A), (B^*, \rmd_B)$ be cochain complexes and $f^k: A^k \to B^k$ a vector
space homomorphism for every $k$. We call $f^*$ \textbf{cochain map} if it commutes with the differentials
i.e. if the following diagram commutes for every $k$:
\[\begin{tikzcd}
	{A^k} & {A^{k + 1}} \\
	{B^k} & {B^{k + 1}}
	\arrow["{\mathrm{d}_A}", from=1-1, to=1-2]
	\arrow["{f^k}"', from=1-1, to=2-1]
	\arrow["{f^{k + 1}}"', from=1-2, to=2-2]
	\arrow["{\mathrm{d}_B}"', from=2-1, to=2-2]
\end{tikzcd}\]
\end{definition}
Cochain maps descend to maps on cohomology in a familiar way:
\begin{definition}
...
context. It still has to be proven that this induced map is actually well-defined:
\begin{proof}
...
that these maps are cochain maps!
\begin{lemma}
...
\end{tikzcd}\]
be a short exact sequence of cochain complexes. Then there exists a connecting morphism
...
Since $a$ depends on choices of representative $c$ and preimage $b$ one has to prove that
$\rmd^*$ is nonetheless well-defined. We are going to start with proving that $[a]$ is
independent of the choice of $b$ by letting $\tilde{b}$ be such that $g^k(\tilde{b}) = c$. Then
$g^k(b - \tilde{b}) = 0$ and by exactness there exists a unique $\hat{a}$ such that
$f^k(\hat{a}) = b - \tilde{b}$, resulting in:
...
Continuing with $c$, let $[\tilde{c}] \in H^k(C^*)$ such that $[\tilde{c}] = [c]$. Then $[c
- \tilde{c}] = 0$ which implies the existence of $\hat{c} \in C^{k - 1}$ such that $\rmd_C
\hat{c} = c - \tilde{c}$. By surjectivity of $g^{k - 1}$ there is then $\hat{b} \in B^{k - 1}$
such that $g^{k-1}(\hat{b}) = \hat{c}$, resulting in:
...
\end{tikzcd}\]
Since $\rmd^*$ does not depend on the choice of $b$ this implies
$\rmd^*([c - \tilde{c}]) = 0$, thus $\rmd^*$ is independent of the choice of representative $c$ and in
particular well-defined.

It remains to show that the sequence is exact. Start by proving that
...
$g^{k - 1}(\hat{b}) = c$. Since $g^*$ is a cochain map the 
...
$f^k(a) = b - \rmd_B \hat{b}$. Since $f^*$ is a cochain map the following diagram commutes:
...
To show its concrete usefulness, we are going to use the Mayer-Vietoris Sequence to calculate
the de Rham Cohomology of $S^1$, a previously quite daunting task. We cover $S^1$ by two open
sets $\U, \V \subseteq S^1$ as shown in TODO. These sets are diffeomorphic to $\RR$ and thus
their cohomology is already known:
\[
	...
	\end{cases}
\]
The intersection $\U \cap \V$ are diffeomorphic to two disjoint copies of $\RR$ and since the
exterior derivative is defined locally it is easy to show that the cohomology of the disjoint
union of two sets is isomorphic to the sum of the cohomologies of the sets.

The Mayer-Vietoris sequence of these spaces is then
% https://q.uiver.app/#q=
\[\begin{tikzcd}[cramped]
	0 & {H^0(S^1)} & {H^0(\mathcal{U}) \bigoplus H^0(\mathcal{V})} & {H^0(\U \cap \V)} & {H^1(S^1)} & 0 \\
...
A big advantage of exact sequences of vector spaces is the possibility of getting quite a lot of information
by counting dimension. That is: we know that $j^*$ is injective and thus its image is 1-dimensional,
thus the image of $i^*$ is $2 - 1 = 1$-dimensional, thus the image of $\rmd^*$ is also 1-dimensional
and since it is also surjective, $H^1(S^1)$ is 1-dimensional.

% But the usefulness of the Mayer-Vietoris sequence does not stop there, we can even find a concrete
% basis of $H^1(S^1)$ by looking at $\rmd^*$. First we have to pick an element that does not get
% mapped to zero by $\rmd^*$ i.e. one that isn't in the image of $i^*$. Since $H^0(\mathcal{U})$
% and $H^0(\mathcal{V})$ consist of the locally constant functions on $\U$ and $\V$ and $i^*$ is
% the subtraction of restrictions, this is just a locally constant function on $\U \cap \V$ that
% is not globally constant e.g. the function that is 1 on the "top"-part 
% constant 
% an element of $H^0(\mathcal{U} \cap \mathcal{V})$
...
% $\U$ and $\V$ e.g. $\mathds{1}_\U$ (the indicator function of $\U$). To calculate $\rmd^*[\mathds{1}_\U]$,
