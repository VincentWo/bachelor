...
starting with the generalization of differentation to more general domains then just open
subsets of $\RR^n$.
...
then reading one of the plethora of proper ones is recommened e.g. \cite{lee_introduction_2013},
...
A \textbf{Topological Manifold} of dimension $n$ is a metrizable and separable\footnote{
	...
	ensures uniqueness of limits while seperability ensures that our spaces don't get "too big".
}
...
\end{definition}
In this context our suggested definition of smoothess makes sense:
\begin{definition}
...
Smoothness is also used to define an equivalence relation on manifolds that is more usefull than
strict equality:
\begin{definition}
...
\]
While this might seem like meaningless notationals tricks it shows that translating the derivative to
arbitrary smooth manifolds starts with finding a usefull definition of $T_p M$ for any smooth manifold $M$.
...
not want to have to choose local coordinates to make sense of $T_p M$. And one does not have to by realising
...
coordinates $(x, \U)$ around $p$ and $(y, \V)$ around $f(p)$ fullfills 
\[
...
\end{definition}
To ensure that this map really exists we have to prove that any map fullfilling this equality for charts
$(x, \U)$ around $p$ and $(y, \V)$ around $f(p)$ also fullfills it for a different choice of charts
...
having to take some of the remaining things as black bloxes with proofs and details left to the earlier
...
maps $X: M \to TM$ which fullfill $X(p) \in T_p M$. This definition includes the old one if one takes
...
manifolds, all that is left is to find a nice decription of these using derivatives. This is a bit more involved:
...
case of general manifolds this is nescessary, but in the case of $\RR$ there is a canonical isomorphism between
...
% of $T_p M$ is going to be denoted as $T_p^* M$, and then similiary to the case of the tangent bundle it is
...
% 	A similiar isomorphism is also possible in the more general case of Riemannian manifolds. These
...
This local notion of volume around $p$ can be thought of assigning volume to parallelopipeds based at $p$.
Of course parallelopipeds do not exist on arbitrary manifolds, but one can approximate them through the use
of tangent vectors i.e. every $n$-tuple of tangent vectors in $T_p M$ can be interpreted as an
$n$-parallelopiped. Thus we want to describe volume using maps from $(T_p M)^n$ to $\RR$, though we also
want them to fullfill some more requirements to make them more "volume-like".

Firstly we are going to require them to map linear dependent tangent vectors to $0$ since these represent
degenerate parallelopipeds. Secondly we want them to be multi-linear, like the normal volume of
parallelopipeds. This has the odd consequence of having to deal with negative "volume", but this can be seen
...
% noticing what one has to prevent. even talk about orientation on a manifolds requires it to be coordinate independent i.e. all of our coordinate changess need to
...
higher dimensional multi-linear maps in a similiar form, which in the special case of anti-symmetric maps
can be achieved in a succint way using the so called \textbf{wedge product}.
...
anti-symmetrice any multi-linear map:
\begin{definition}
...
\begin{theorem}
	The wedge product fullfills the following properties:
	\begin{itemize}
		\item
...
% We are also going to use this to introduce a usefull abuse of notation: Though technically
...
$p \in M$ a function from the cartesean product of 0 vector spaces (which is by definition just a point) to
...
unique linear operator mapping $k$-forms to $k + 1$-forms that fullfills:
\begin{itemize}
	...
	\item It satisfies the graded Leibniz rule
		...
		\]
		for any homogenous forms $\alpha, \beta$ with $k$ being the degree of $\alpha$
...
\begin{proof}
	...
	already determines $\rmd \omega$ since $\rmd$ being linear, fullfilling the graded Leibniz rule and being
	...
	actually fullfills all required properties.
	...
	Because of linearity it is enough to prove the Leibniz rule for homogenous terms
	$f \rmd x^{i_1} \wedge \dots \wedge \rmd x^{i_k}$ and $g \rmd x^{j_1} \wedge \dots \wedge x^{j_\ell}$,
	which we are going to abbreviate as $f \rmd x^I$ and $g \rmd x^J$ for multiindices $I,J$.
...
% only works because of our orientation assumption, which guarentees that $\det(D(y \circ x^{-1}))$ is
% positive.
...
% \]
% whre $\{\rho_\alpha\}_{\alpha \in I}$ is a partition of unity subordinate to the charts of $M$.
% \end{definition}
...
% \begin{definition}
% We call a smooth map $f: M \to N$ an \textbf{embedding} if it is an homeomorphism ontos its image (i.e. $f: M \to f(M)$ is a bijection with
% continous inverse) and its tangent map $Tf: TM \to TN$ is injective for every restriction $T_p f: T_p M \to T_{f(p)} N$.
% \end{definition}
...
% These maps characterice smooth submanifolds:
% \begin{definition}
...
% That this definition is independent from the choice of embedding follows from a similiar argument as the indepence of the integral from a
...
% Without further ado:
% \begin{definition}
% A topological manifold with boundary is a metrizable and seperable topological space $X$ such that every
% point $p$ has an open neighborhood that is homeomorphic to an open subset of the $n$-th halfspace i.e. $\mathbb{H}^n = [0, \infty) \times \RR^{n - 1}$
% \end{definition}
...
% and then note that this property is fullfilled. But nontheless it is helpfull to have
% this geometric interpretation in mind, not just as a property that the exterior derivative
% happens to fullfill, but as a definition.
...
Another helpfull statement also follows immeaditly from the definition:
\begin{lemma}
...
simplest of spaces since it involes solving differential equations. Luckily there is major computational
...
by restricting the charts of $M$ to $\U$. Similiarly the disjoint union of two smooth manifolds can also be
...
inclusion of $\U \cap \V$ into $\U$ (and then into the disjoint union), similiar with $i_\V$. All of these
...
\]
\end{definition}
Note that similiar as with the pullback $\reallywidetilde{f^* \circ g^*} = \tilde{f}^* \circ \tilde{g}^*$ and
...
\end{align*}
The elements of $\ker(\rmd^k)$ are called $k$-cocyles of $A^*$ and the elements of $\Img(\rmd^{k - 1})$
$k$-coboundaries of $A^*$.
\end{definition}
...
\end{proof}
As an immeadiate consequence we can now proof that pullbacks of smooth maps descend to cohomology by proving
...
The notion of (short) exact sequences seamingless translates to cochain complexes, finally allowing us to
...
Since $a$ depends on choices of representant $c$ and preimage $b$ one has to prove that $\rmd^*$ is
nontheless well-defined. We are going to start with proving that the choice of $b$ does not matter by choosing
...
$\rmd^*([c - \tilde{c}]) = 0$, thus $\rmd^*$ is independent of the choice of representant $c$ and in
...
To show its concrete usefullness, we are going to use the Mayer-Vietoris Sequence to calculate the de-Rham
...
\end{tikzcd}\]
where we have removed all superfluos cohomologies that are known to be trivial.
...
$\U$ and $\V$ e.g. $\mathbb{1}_\U$ (the indicator funciton of $\U$). To calculate $\rmd^*[\mathds{1}_\U]$,
